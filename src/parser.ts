...
function splitFrontmatter(raw: string): { fm: Record<string, unknown>; body: string } {
  ...
  try {
    ...
    console.warn("DoKKi yaml parse error", e);
  }
  return { fm, body };
}
...
function parseBody(body: string): ParsedBody {
  ...
  let externalQuoteLines: string[] = [];
  const pages: PageExcerpt[] = [];

  let current: { page: number; buf: string[] } | null = null;
  let inExternalQuote = !current;

  for (const line of lines) {
    const pm = line.match(PAGE_HEADER);
    if (pm) {
      if (current) {
        pages.push(finalizePage(current.page, current.buf));
      }
    ...
    if (current) {
      ...
      if (/^\s*>/.test(line)) {
        ...
        // non-blockquote line after we started collecting → stop
        inExternalQuote = false;
      }
    }
  }
